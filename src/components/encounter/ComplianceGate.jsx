...
export default function ComplianceGate({ patient, onFinalize }) {
  ...
  const items = [
    ...(needs97166 ? [{
      ...
      desc: 'Required for CPT 97166 — Initial Evaluation',
      required: true,
      badge: 'blue',
    }] : []),
    ...(needs96112 ? [{
      ...
      desc: 'Required for CPT 96112/96113 — Standardized Assessment',
      required: true,
      badge: 'purple',
    }] : []),
    {
      ...
      desc: 'Therapist has reviewed and approved all suggested goals',
      required: true,
      badge: 'green',
    },
    {
      ...
      desc: 'I confirm this note reflects my clinical judgment and is accurate',
      required: true,
      badge: 'orange',
    },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <motion.div
        ...
        className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${allComplete ? 'bg-emerald-50 border-emerald-200' : 'bg-orange-50 border-orange-200'}`}
      >
        {allComplete
        ...
        <div className="flex-1">
          <p className={`text-sm font-semibold ${allComplete ? 'text-emerald-700' : 'text-orange-700'}`}>
            {allComplete ? 'All compliance requirements met' : `${completedCount} of ${requiredItems.length} requirements complete`}
          </p>
          <p className="text-xs text-slate-500 mt-0.5">
            {allComplete ? 'Note is ready to finalize' : 'Complete all items below before finalizing the note'}
          </p>
        </div>
          ...
          >
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                ...
                <Badge variant={item.badge}>{item.required ? 'Required' : 'Optional'}</Badge>
              </div>
      ...
      <AnimatePresence>
        {!allComplete && (
          ...
          >
            ...
            <p className="text-xs text-red-600">Note finalization is blocked until all required items are completed.</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      >
        {allComplete
          ? <><Unlock size={16} /> Finalize & Sign Note</>
          : <><Lock size={16} /> Complete All Requirements to Finalize</>
        }
      </Button>
    </div>
  );
